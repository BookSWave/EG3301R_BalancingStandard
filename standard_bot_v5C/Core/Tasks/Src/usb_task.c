...
uint8_t motor_config_mode(uint8_t skip) {
	...
	uint16_t send_len;
	if (skip == 0) {
		safety_toggle = 1; //triggers safety toggle so if motors were moving it won't move until safety-untoggled
		...
		send_len = sprintf(send_buffer, "Motor config mode\n"
				"Current RPM: %u\n"
				"Enter e to exit\n"
	...
	if (received_msg == pdTRUE) {
		if (*usb_input_buffer == 'e' || *usb_input_buffer == 'E') {
			vTaskDelay(5);
			send_len = sprintf(send_buffer, "Exiting motor config mode\n");
			CDC_Transmit_FS(send_buffer, send_len);
			...
			int new_pwm_val;
			sscanf(usb_input_buffer, "%d",&new_pwm_val );
			if (new_pwm_val >= -4000 && new_pwm_val<= 4000){
				send_len = sprintf(send_buffer, "New speed value set to %d\n",new_pwm_val);
				CDC_Transmit_FS(send_buffer, send_len);
				referee_limiters.feeding_speed = new_pwm_val/FEEDER_SPEED_RATIO;
				vTaskResume(movement_control_task_handle); //stops all other tasks from working...to be safe
				vTaskResume(gimbal_control_task_handle);
				vTaskResume(launcher_control_task_handle);
				return 0;
			} else {
				send_len = sprintf(send_buffer, "val not within range, enter again pls\n");
				CDC_Transmit_FS(send_buffer, send_len);
				return 1;
			}
		}
		...
		send_len = sprintf(send_buffer, "No message received, exiting motor config mode\n");
		CDC_Transmit_FS(send_buffer, send_len);
