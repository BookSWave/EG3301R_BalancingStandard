...
void gimbal_control_task(void *argument) {
	...
	while (1) {
		// xEventGroupWaitBits(gimbal_event_group, 0b11, pdTRUE, pdTRUE, portMAX_DELAY);
		...
		if (gimbal_ctrl_data.enabled) {
			...
			can_motors[ROLL_MOTOR_ID - 1].rpm_pid.output = 0;
			motor_send_can(can_motors, PITCH_MOTOR_ID, YAW_MOTOR_ID, ROLL_MOTOR_ID, 0);
		}
 ...
 */
void gimbal_control(motor_data_t *pitch_motor, motor_data_t *yaw_motor, motor_data_t *roll_motor) {
	static float prev_pit;
	static float prev_yaw;
	static float prev_rol;
...
#else
	float rel_pitch_angle = gimbal_ctrl_data.pitch * 3.1-1.32; // insert function where input desired gimbal pitch angle and output corresponding motor angle
...
//			imu_heading.yaw, &prev_yaw);
    ...
    // Roll control
//    float rel_roll_angle = roll_motor->angle_data.adj_ang + gimbal_ctrl_data.roll - imu_heading.rol;
//    if (rel_roll_angle > roll_motor->angle_data.phy_max_ang) {
//        rel_roll_angle = roll_motor->angle_data.phy_max_ang;
//        roll_lim = 1;
//    }
//    if (rel_roll_angle < roll_motor->angle_data.phy_min_ang) {
//        rel_roll_angle = roll_motor->angle_data.phy_min_ang;
//        roll_lim = 1;
//    }
//    if (roll_lim == 1) {
//        gimbal_ctrl_data.roll = rel_roll_angle + imu_heading.rol - roll_motor->angle_data.adj_ang;
//    }
//	yangle_pid(gimbal_ctrl_data.roll,imu_heading.rol, roll_motor,
//			imu_heading.rol, &prev_rol);

	float rel_roll_angle = roll_motor->angle_data.adj_ang
			+ gimbal_ctrl_data.roll - imu_heading.rol;
	if (rel_roll_angle > roll_motor->angle_data.max_ang) {
		rel_roll_angle = roll_motor->angle_data.max_ang;
		roll_lim = 1;
	}
	if (rel_roll_angle < roll_motor->angle_data.min_ang) {
		rel_roll_angle = roll_motor->angle_data.min_ang;
		roll_lim = 1;
	}
	if (roll_lim == 1) {
		gimbal_ctrl_data.roll = rel_roll_angle + imu_heading.rol
				- roll_motor->angle_data.adj_ang;
	}
