...
#ifdef ROLL_MOTOR_ID
	motor_id = ROLL_MOTOR_ID - 1;
	can_motors[motor_id].motor_type = TYPE_GM6020;
	set_motor_config(&can_motors[motor_id]);
	can_motors[motor_id].angle_data.gearbox_ratio = 0;
	can_motors[motor_id].angle_data.center_ang = ROLL_CENTER;
	can_motors[motor_id].angle_data.max_ang = ROLL_MAX_ANG;
	can_motors[motor_id].angle_data.min_ang = ROLL_MIN_ANG;
	can_motors[motor_id].angle_data.phy_max_ang = 2 * PI;
	can_motors[motor_id].angle_data.phy_min_ang = -2 * PI;
	can_motors[motor_id].angle_data.wheel_circ = 0;
	can_motors[motor_id].angle_pid.kp = ROLL_ANGLE_KP;
	can_motors[motor_id].angle_pid.ki = ROLL_ANGLE_KI;
	can_motors[motor_id].angle_pid.kd = ROLL_ANGLE_KD;
	can_motors[motor_id].angle_pid.int_max = ROLL_ANGLE_INT_MAX;
	can_motors[motor_id].angle_pid.max_out = ROLL_MAX_RPM;
	can_motors[motor_id].rpm_pid.kp = ROLLRPM_KP;
	can_motors[motor_id].rpm_pid.ki = ROLLRPM_KI;
	can_motors[motor_id].rpm_pid.kd = ROLLRPM_KD;
	can_motors[motor_id].rpm_pid.int_max = ROLLRPM_INT_MAX;
	can_motors[motor_id].rpm_pid.max_out = ROLL_MAX_CURRENT;
#endif
}
